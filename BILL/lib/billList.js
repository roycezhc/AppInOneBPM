...
function ListManager(paraObjIn) {
    ...
    var getColDataObj = function (thObj, backSetDataObj) {
        ...
        if (datastr != "" && datastr != undefined) {
            ...
            if (dataObj.show != undefined) {
                obj.show = dataObj.show;
            } else if (backSetObj.show != undefined) {
                obj.show = backSetObj.show;
            }

            if (dataObj.sort != undefined) { obj.sort = dataObj.sort; } else if (backSetObj.sort != undefined) {
                obj.sort = backSetObj.sort;
            }
            if (dataObj.ctype != undefined) { obj.ctype = dataObj.ctype; } else if (backSetObj.ctype != undefined) {
                obj.ctype = backSetObj.ctype;
            }
            if (dataObj.find != undefined) { obj.find = dataObj.find; } else if (backSetObj.find != undefined) {
    ...
    this.newRecord = function (url, morePName, type) {
        ...
        if (type == 1 || type == undefined) {
            ...
            re = window.showModalDialog(url, "modlg_window", "dialogHeight=" + _recordWindowHeight + "px; dialogWidth=" + _recordWindowWidth + "px;dialogLeft:;dialogTop:; status=no;"); if (!re) { re = window.returnValue; } //jin-shenjian chrome刷新父画面
        
        }
        else if (type == 2) {
            url += "openwin=1&r=" + Math.random();
            re = window.open(url, "new_window", "height:" + _recordWindowHeight + ",width:" + _recordWindowWidth + ",top:,left:,alwaysRaised:yes,depended:yes,location:no,menubar:no,resizable:yes,toolbar:no,hotkeys:no,z-look:yes");
        }
        else if (type == 3) {
    ...
    this.showRecord = function (url, morePName, type) {
        if (type == undefined) { type = 1; }
        if (url.indexOf("?") == -1) {
            url += "?";
        } else {
        ...
        url += "KeyID=" + valObj.selKeyID; // alert(url);
        ...
        if (type == 1 || type == undefined) {
            ...
            url += "&openwin=1&r=" + Math.random();
            re = window.open(url, "edit_window", "height:" + _recordWindowHeight + ",width:" + _recordWindowWidth + ",top:,left:,alwaysRaised:yes,depended:yes,location:no,menubar:no,resizable:yes,toolbar:no,hotkeys:no,z-look:yes");
        }
        else if (type == 3) {
            url += "&preurl=" + escape(location.href) + "&r=" + Math.random();
            location.href = url;
    ...
    this.editRecord = function (url, morePName, type) {
        if (type == undefined) { type = 1; }
        if (url.indexOf("?") == -1) {
            url += "?";
        } else {
        ...
        if (type == 1 && re != undefined && re.indexOf("{valObj:") != -1) {
            ...
            var tds = trObj.children();
            var curTDS = $("#" + this.paraObj.listCID).find("tbody:last").find("tr[class*='sel']").children();
    ...
    this.getSelectedIDS = function () {
        var cks = $("#" + this.paraObj.listCID).find("tbody:last").find("input[type='checkbox']");
        var arr = new Array();
...
function _selAll(me) {
    ...
    var bodyObj = tdObj.parent().parent().next();
   
    var trs = bodyObj.children();
    var colIndex = tdObj.parent().children().index(tdObj);
    var ck = me.checked;
    trs.each(function (i) {
