var SocketIp = "kf.shengyt.com";
var TimeInterval;
var getType = 1;
var ajaxAddress = "http://kf.shengyt.com/";
...
ChatFloatFactory.prototype = {
    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                console.log(obj);
                if (obj && obj.shengytid && obj.shengytid.length > 0) {
                    t.initialize(options, obj.shengytid, obj.msgcount, obj.socket);
                    e.setCookie('_useright_' + options.AId, '{"aid":' + options.AId + ',"shengytid":"' + obj.shengytid + '","socket":' + obj.socket + ',"openid":"' + options.openid + '"}', 300 * 1000);
                    e.setCookie('_3GUser_' + options.AId, obj.shengytid, 3600 * 24 * 10000 * 1000);
                }
            }
        }, function (status) { });
        ...
        ajax.get(ajaxAddress + "Ajax/CustomerChatAjax.aspx?action=userright&AId=" + options.AId + "&openid=" + openid + "&shengytid=" + shengytid, true);
    },
    SocketMessage: function (AId, shengytid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatSocketMessage&shengytid=' + shengytid + "&AId=" + AId, true);
    },
    StartPolling: function (AId, shengytid) {
        getType = 2;
        clearInterval(TimeInterval);
        TimeInterval = setInterval(GetPolling, 10000);
        this.GetPolling(AId, shengytid);
    },
    GetPolling: function (AId, shengytid) {
        var e = this;
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
        ...
        ajax.get(ajaxAddress + 'Ajax/CustomerChatAjax.aspx?action=GetFloatPolling&shengytid=' + shengytid + "&AId=" + AId, true);
    },
    ...
    LocationSource: function (cModel) {
        ...
        var openid = options.openid ? options.openid : "";
        var param = "AId=" + cModel.aid + "&openid=" + cModel.openid + "&shengytid=" + cModel.shengytid + "&nickname=" + cModel.nickname + "&markname=" + cModel.markname + "&headurl=" + cModel.headurl;
...
window.ChatFloat = function (options) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId && objStorage.shengytid) {
                var socket = 0;
                if (objStorage.socket) {
                    socket = objStorage.socket;
                }
                e.initialize(options, objStorage.shengytid, 0, socket);
                Tools.SocketMessage(options.AId, objStorage.shengytid);
            }
            else {
...
ChatFloat.prototype = {
    initialize: function (options, shengytid, msgcount, socket) {
        var e = this;
        ...
        mydiv.style.background = "url('http://kf.shengyt.com/images/MobileChatFloat.png') no-repeat 0px 0px";
        ...
        if (socket == 1) {
            Tools.loadjscssfile("http://" + SocketIp + ":8889/socket.io/socket.io.js", "js", function () { e.socketconnect(options, shengytid); });
        }
    ...
    socketconnect: function (options, shengytid) {
        try {
            ...
            socket.on("connect", function () {
                ...
                socket.on('dataChange', function (json) {
                    if (json) {
                        if (json.msgCount == 1) {
                            Tools.SocketMessage(options.AId, shengytid);
                        }
                    }
                });
            });
