    ...
    AjaxRight: function (t, options) {
        ...
        var ajax = new Ajax(function (responseText, responseXML) {
            if (responseText) {
                ...
                if (obj) {
                    e.setCookie('_useright_' + options.AId, '{"show":' + obj.succ + ',"aid":' + options.AId + ',"shengytid":"' + obj.shengytid + '","socket":' + obj.socket + ',"openid":"' + options.openid + '"}', 300 * 1000);
                    if (obj.succ && obj.shengytid && obj.shengytid.length > 0) {
    ...
    if (options && options.AId) {
        ...
        if (strStoreData && strStoreData.length > 0) {
            ...
            if (objStorage && objStorage.aid == options.AId) {               
                if (objStorage.show && objStorage.shengytid) {
                    var socket = 0;
