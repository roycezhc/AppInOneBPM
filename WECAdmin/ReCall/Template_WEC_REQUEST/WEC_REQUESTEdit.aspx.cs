...
public partial class WEC_REQUESTEdit : BaseAdminPage
{
    ...
    string Editor = @"<!--Switch-->
                    <!--Case InputString--><!--文字-->
                    <input id='{字段控件名}' type='text' runat='server' ck='{type:{字段js类型枚举},min:0,max:{字段标签长度},need:{字段是否必须}}'/>
                    <!--Case SmallText-->
                    <textarea id='{字段控件名}' class='smalltextarea' cols='1' rows='1' style='height:20px;width:300px;overflow:hidden;' 
                            runat='server' ck='{need:{字段是否必须},len:{字段标签长度},type:{字段js类型枚举}}' />
                    <!--Case Text-->
                    <script type='text/javascript'>
                        $(function () {
                            var kindEditor = new creatKindEditor('#<%={字段控件名}.ClientID%>');
                        });
                    </script>
                    <input id='{字段控件名}' type='text'  runat='server' style='width:668px;height:230px;' ck='{need:{字段是否必须},len:{字段标签长度},type:{字段js类型枚举}}'/>
                    <!--Case HTML-->
                    <FCKV2:FCKeditor ID='{字段控件名}' runat='server' Height='250' Width='400' ToolbarSet='Basic' />
                    <!--Case SelectDrop--><!--Case SelectDrop-->
                    <select id='{字段控件名}' runat='server' ck='{need:{字段是否必须},len:{字段标签长度},type:{字段js类型枚举}}'>
                    </select>
                    <!--Case Select_RelationUser-->
                    <input id='{字段控件名}' type='hidden'  runat='server'/>
                    <input id='{字段控件名}_NAME' class='selshowinput' readonly='readonly' style='width:70%;float:left;' runat='server' type='text' />
                    <input class='sel' type='button' onclick='sel(this,""{首字母大写关联表名}"");' value='' style='float:left;' />                    
                    <!--Case SelectMultiple-->
                    <!--Case CheckBoxList-->
                    <input id='{字段控件名}' type='checkbox' class='ckb' runat='server' />
                    <!--Case RadioBoxList--> 
                    <input id='{字段控件名}' type='radio' class='ckb' runat='server' />
                    <!--Case InputDateTime-->
                    <input id='{字段控件名}' type='text' readonly='readonly' onclick='setday(this)' runat='server' ck='{need:{字段是否必须},len:{字段标签长度},type:3}' />
                    <!--Case InputDate-->
                    <input id='{字段控件名}' type='text' readonly='readonly' onclick='setday(this)' runat='server' ck='{need:{字段是否必须},len:{字段标签长度},type:3}'/>
                    <!--Case FileUpUpDoc-->
                    <uc1:UpFile ID='UpFile{首字母大写字段名}' runat='server' />
                    <!--Case InputPassWord--> 
                    <input id='{字段控件名}' type='password' runat='server' ck='{need:{字段是否必须},len:{字段标签长度},type:{字段js类型枚举}}' />
                    <!--Case FileUpImage-->              
                    <script type='text/javascript'>
                        $(function () {
                            var smImage = new creatSmImage('#btn{字段控件名}', '#<%={字段控件名}.ClientID%>', '#<%=hid{字段控件名}.ClientID%>');
                        });
                    </script>
                    <div class='controls'>
					    <img id='{字段控件名}' runat='server' style='max-height:100px;vertical-align: middle;'>
                        <span class='insertimage'><a id='btn{字段控件名}'>选择图文封面</a></span>  建议大小(宽720高400)
                        <input type='hidden' id='hid{字段控件名}' runat='server' />
                    </div>
                <!--EndSwitch-->";
