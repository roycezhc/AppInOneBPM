...
public partial class WECAdmin_WebSite_Menu_MenuEdit : BaseAdminPage
{
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (keyid != "")
            {
                ...
                count = BLLTable<WEC_MENU>.Update(valObj, WEC_MENU.Attribute.ID);
            }
            else
            {
                count = BLLTable<WEC_MENU>.Insert(valObj);
    ...
    public static string GetNewMenuID(string parentid)
    {
        ...
        string code = "0";
        WEC_MENU objCond = new WEC_MENU();
        objCond.PID = parentid;
        object ooo = BLLTable<WEC_MENU>.Max(WEC_MENU.Attribute.ID, objCond);//010203
        ...
        else
        {
            ...
            else if (newlasttwo.Length > 2)//编号后两位达到上限FF,只能遍历是否有空位
            {
                ...
                objCond.OrderBy(WEC_MENU.Attribute.ID, Order.Asc);

                DataTable dt = BLLTable<WEC_MENU>.SelectForDataTable(new WEC_MENU(), objCond);
