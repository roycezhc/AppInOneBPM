//图片预览
//sender:上传对象this  imgID:预览对象,max_width:最大宽图，max_height:最大高度
function onUploadImgChange(sender, imgID, max_width, max_height, apppath) {
  
    if (!isImage(sender.value, 0)) {
        alert("请上传图片！");
        return false;
    }

    var ip = new ImagePreview($$(sender.id), $$(imgID), {
        maxWidth: max_width,
        maxHeight: max_height,
        action: apppath+"Lib/Preview_Plug/ImagePreview.ashx"
    });
...
//上传文件是否是图片或TXT，XLS文件   
//str:文件路径  type: 0为对图片操作，1为对TXT，XLS
function isImage(str, type) {
    var ext = str.lastIndexOf(".");
    var extName = str.substr(ext + 1, str.length).toLowerCase();

    var strAllowExt = "jpg,jpeg,gif,png,bmp";
    if (type == 1) {
        strAllowExt = "txt,xls";
    }
    if (strAllowExt.indexOf(extName) > -1) {
        return true;
    }
    return false;
}
