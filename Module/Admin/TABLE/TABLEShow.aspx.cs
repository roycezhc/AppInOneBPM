...
public partial class TF_TABLEShow2 : AgileFrame.AppInOne.Common.BasePage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            try
            {
                if (!string.IsNullOrEmpty(Request["TB_ID"]) || !string.IsNullOrEmpty(Request["ID"]))
                {
                    string tbid = Request["TB_ID"];
                    if (!string.IsNullOrEmpty(Request["ID"]))
                        tbid = Request["ID"];
                    valObj = BLLTable<TF_TABLE>.Factory(conn).GetRowData(TF_TABLE.Attribute.TB_ID, tbid);
