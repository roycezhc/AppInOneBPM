<%@ Page Title="" Language="C#" MasterPageFile="~/Themes/Blank.master" AutoEventWireup="true" Inherits="SYS_BILL_INDEXTB" CodeFile="IndexTB.aspx.cs" %>
            ...
            var tv = new TV.Tree("treeBill", {
                selNodeHandle: function (tn) {
                    ...
                    if (ntype == "bill" || ntype == "bill-mx") {
                        $("#ifrMain").attr("src", "BILLEditTB.aspx?edit=1&" + tn.getDataPara());
                    }
                    if (ntype == "col") {
                        $("#ifrMain").attr("src", "ColEditTB.aspx?edit=1&" + tn.getDataPara());
                    }
                    return false;
                },                
                pageBack: 'BILLBackTB.aspx?DEV2=' + $("select[id$='txtDEV']").val(), //设置异步处理的后台页面地址
                showDelete: true,
                checkBox: 0
            });
            //菜单项
            tv.addMenu("addBill", "新建表", function (tn) {
                $("#ifrMain").attr("src", "BILLEditTB.aspx?" + tn.getDataPara());
            });
            tv.addMenu("editBill", "字段列表", function (tn) {
                $("#ifrMain").attr("src", "COLManage.aspx?" + tn.getDataPara());
            });
            tv.addMenu("addCol", "新建字段", function (tn) {
                $("#ifrMain").attr("src", "ColEditTB.aspx?" + tn.getDataPara());
            });
