<%@ Page Title="排班管理" Language="C#" MasterPageFile="~/Themes/Blank.master" AutoEventWireup="true" Inherits="CheckWork_DeptSchedulingManage" CodeFile="DeptSchedulingManage.aspx.cs" %>
...
<script type="text/javascript">
    ...
    $(function () {
        ...
        $("#ifrMain")[0].src = "V_SchedulingSchClass/DeptSchedulingShow.aspx?r=" + Math.random();
    });
    ...
    function gotoPageByNode(tn) {
        ...
        if (jsonStr.indexOf("ntype:'org'") != -1) {
            $("#ifrMain")[0].src = "V_SchedulingSchClass/DeptSchedulingShow.aspx?orgid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        else if (jsonStr.indexOf("ntype:'dept'") != -1) {
            $("#ifrMain")[0].src = "V_SchedulingSchClass/DeptSchedulingShow.aspx?deptid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        else if (jsonStr.indexOf("ntype:'man'") != -1) {
            $("#ifrMain")[0].src = "V_SchedulingSchClass/DeptSchedulingShow.aspx?staffid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        $(".right").unmask();
    }
