<%@ Page Title="排班管理" Language="C#" MasterPageFile="~/Themes/Blank.master" AutoEventWireup="true" Inherits="CheckWork_DeptV_SchedulingManage" CodeFile="DeptV_SchedulingManage.aspx.cs" %>
...
<script type="text/javascript">
    ...
    function gotoPageByNode(tn) {
        ...
        if (jsonStr.indexOf("ntype:'org'") != -1) {
            $("#ifrMain")[0].src = "DeptV_SchedulingSchClassBuild.aspx?orgid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        else if (jsonStr.indexOf("ntype:'dept'") != -1) {
            $("#ifrMain")[0].src = "DeptV_SchedulingSchClassBuild.aspx?deptid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        else if (jsonStr.indexOf("ntype:'man'") != -1) {
            $("#ifrMain")[0].src = "DeptV_SchedulingSchClassBuild.aspx?staffid=" + objData.id + "&name=" + encodeURI(objData.name) + "&r=" + Math.random();
        }
        $(".right").unmask();
    }
...
<div class="main" style="width:980px">
   ...
   <div class="lrlayout">
    <div class="left" style="width:200px;height:auto;">
     ...
     <iframe src="DeptV_SchedulingSchClassBuild.aspx<%=idstr %>" id="ifrMain" frameborder="0"></iframe>
     </div>
    </div>
</div>
</asp:Content>
