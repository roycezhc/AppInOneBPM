...
public partial class WF_CHECKER_TPLEdit : AgileFrame.AppInOne.Common.BaseAdminPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        Page.Title = title;
        if (!string.IsNullOrEmpty(Request["CHECKER_ID"]))
        {
            keyid = Request["CHECKER_ID"];
        }
        ...
        if (!IsPostBack)
        {
            ...
            txtSTATUS.Items.AddRange(FormHelper.GetListItem(WF_CHECKER_TPL.Attribute.STATUS));
            ...
            //txtCHECKER_TYPE.Items.AddRange(FormHelper.GetListItem(WF_CHECKER_TPL.Attribute.CHECKER_TYPE));
            if (txtSTATUS.Items.Count > 1)
            {
                txtSTATUS.SelectedIndex = 1;
            }
            //if (txtCHECKER_TYPE.Items.Count > 1)
            //{
            //    txtCHECKER_TYPE.SelectedIndex = 1;
            //}
            //while (txtCHECKER_TYPE.Items.Count > 4)
            //{
            //    txtCHECKER_TYPE.Items.RemoveAt(txtCHECKER_TYPE.Items.Count - 1);
            //}
            ...
            try
            {
                if (keyid != "")
                {
                    ...
                    valObj = BLLTable<WF_CHECKER_TPL>.Factory(conn).GetRowData(WF_CHECKER_TPL.Attribute.CHECKER_ID, keyid);
                    if(valObj==null) return ;

                    if (valObj.CHECKER_TYPE == WFEnum.CheckerType.SelStaff.ToString("d"))
                    {
                        wucSelStaff1.Staff_NAME = valObj.STAFF_NAME;
                        wucSelStaff1.Staff_ID = decimal.Parse(valObj.STAFF_ID);
                    }
                    else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.SelDept.ToString("d"))
                    {
                        wucSelDept1.DEPT_NAME = valObj.STAFF_NAME;
                        wucSelDept1.DEPT_ID = valObj.STAFF_ID.ToString();
                       
                    }
                    else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.SelHRole.ToString("d"))
                    {
                        wucSelStation1.STATION_NAME = valObj.STAFF_NAME;
                        wucSelStation1.STATION_ID = valObj.STAFF_ID.ToString();

                    }
                    else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.SelLevel.ToString("d"))
                    {
                        SelLevel.Value = valObj.STAFF_ID.ToString();
                    }
                    else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.SelLeaderLevel.ToString("d"))
                    {
                        SelLeaderLevel.Value = valObj.STAFF_ID.ToString();
                    }
                    //txtCHECKER_ID.Value = Convert.ToString(valObj.CHECKER_ID);//Convert.ToInt32                
                    ...
                    txtCHECKER_TYPE.Value = Convert.ToString(valObj.CHECKER_TYPE);//Convert.ToString                
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (txtCHECKER_TYPE.Value != "")
                valObj.CHECKER_TYPE = Convert.ToString(txtCHECKER_TYPE.Value);
            ...
            else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.LeaderDirect.ToString("d"))
            {
                valObj.STAFF_NAME = "直接领导";
            }
            else if (valObj.CHECKER_TYPE == WFEnum.CheckerType.LeaderInDirect.ToString("d"))
            {
            ...
            if (keyid != "")
            {
                valObj.CHECKER_ID = Convert.ToInt32(keyid);
                count = BLLTable<WF_CHECKER_TPL>.Factory(conn).Update(valObj, WF_CHECKER_TPL.Attribute.CHECKER_ID);
            }
            else
            {
                valObj.CONDITION = "";
                ...
                count = BLLTable<WF_CHECKER_TPL>.Factory(conn).Insert(valObj, WF_CHECKER_TPL.Attribute.CHECKER_ID);
                keyid = valObj.CHECKER_ID.ToString();
