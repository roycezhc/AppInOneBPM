...
public partial class WEC_CUSTOM_CATEBANNEREdit : BaseAdminPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            try
            {
                if (keyid != "")
                {
                    valObj = BLLTable<WEC_CUSTOM_CATEBANNER>.GetRowData(WEC_CUSTOM_CATEBANNER.Attribute.ID, keyid);
                    if (valObj == null) return;


                    txtID.Value = Convert.ToString(valObj.ID);//Convert.ToDecimal                

                    txtAID.Value = Convert.ToString(aid);//Convert.ToDecimal                
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (keyid != "")
            {
                ...
                count = BLLTable<WEC_CUSTOM_CATEBANNER>.Update(valObj, WEC_CUSTOM_CATEBANNER.Attribute.ID);
            }
            else
            {
                count = BLLTable<WEC_CUSTOM_CATEBANNER>.Insert(valObj, WEC_CUSTOM_CATEBANNER.Attribute.ID);
